...
from mod.functions import get_designsphysics_path
...
def refocus_cwd():
    ...
    chdir(get_designsphysics_path())


def are_executables_bundled():
    """ Returns if the DualSPHysics executable directory exists"""
    dsph_execs_path = "{}/dualsphysics/bin/".format(get_designsphysics_path())
...
def ensure_process_is_executable_or_fail(cli_path: str) -> None:
    ...
    if platform in ("linux", "linux2"):
        if path.exists(cli_path):
            ...
            raise RuntimeError(
                __("The executable {} does not exist.").format(
                    cli_path))
