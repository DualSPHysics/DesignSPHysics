...
SIZE_OFFSET=9
class IntValueInput(QtWidgets.QWidget):
    focus = QtCore.Signal(str)
    value_changed =QtCore.Signal()
    help_text = ""

    def __init__(self, min_val=-10e8,max_val=10e8,minwidth=DEFAULT_MIN_WIDGET_WIDTH, maxwidth=DEFAULT_MAX_WIDGET_WIDTH,parent=None,value=0):#maxwidth=45
        super().__init__(parent=parent)
        self._value=0
        self.quantity_box = FreeCADGui.UiLoader().createWidget("Gui::IntSpinBox")
        if value:
            self._value = value
        ...
        self.quantity_box.setProperty('decimals', 0)
