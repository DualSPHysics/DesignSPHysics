...
from mod.widgets.dock.special_widgets.moorings.moordynplus_parameters_dialog import MoorDynPlusParametersDialog
...
class MooringsConfigurationDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.moordynplus_parameters_data: MoorDynPlusConfiguration = deepcopy(Case.the().moorings.moordynplus_configuration)  # Result of the MoorDynPlusParametersDialog
        ...
        self.configuration_method_label: QtWidgets.QLabel = QtWidgets.QLabel(__("Configuration method for MoorDynPlus:"))
        ...
        # Configure MoorDynPlus big button
        self.configure_moordynplus_button: QtWidgets.QPushButton = QtWidgets.QPushButton(__("Configure MoorDynPlus Parameters"))
        ...
        self.root_layout.addWidget(self.configure_moordynplus_button)
        ...
        self.configure_moordynplus_button.clicked.connect(self._on_configure_moordynplus_parameters)
    ...
    def _on_enabled_change(self, index) -> None:
        ...
        self.configure_moordynplus_button.setEnabled(index == 1)
    ...
    def _on_configuration_method_change(self, index) -> None:
        ...
        if index == 0:
            self.xml_file_selection_widget.hide()
            self.configure_moordynplus_button.show()
        if index == 1:
            self.xml_file_selection_widget.show()
            self.configure_moordynplus_button.hide()
    ...
    def _fill_data(self) -> None:
        ...
        self.xml_file_selection_edit.setText(case.moorings.moordynplus_xml)
    ...
    def _on_configure_moordynplus_parameters(self) -> None:
        """ Opens up the MoorDynPlus configuration parameters dialog. """
        ...
        for row_num in range(0, self.floating_selection_table.rowCount()):
            ...
            if target_widget.use_checkbox.isChecked():
                list_of_matching_bodies: list = list(filter(lambda body: body.ref == target_widget.mkbound, self.moordynplus_parameters_data.bodies))
                new_body: MoorDynPlusBody = list_of_matching_bodies[0] if list_of_matching_bodies else MoorDynPlusBody(target_widget.mkbound)
                new_selected_bodies.append(new_body)

        self.moordynplus_parameters_data.bodies = list(new_selected_bodies)
        if not self.moordynplus_parameters_data.bodies:
            error_dialog(__("You must at least select one floating body to configure MoorDynPlus"))
            return

        MoorDynPlusParametersDialog(self.moordynplus_parameters_data)
    ...
    def _on_ok(self) -> None:
        ...
        if self.enabled_selector_combobox.currentIndex() == 1:
            ...
            Case.the().moorings.moordynplus_xml = self.xml_file_selection_edit.text()
            ...
            Case.the().moorings.moordynplus_configuration = deepcopy(self.moordynplus_parameters_data)
        else:
