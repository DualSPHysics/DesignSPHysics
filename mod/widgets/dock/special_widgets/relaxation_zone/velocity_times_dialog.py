...
class VelocityTimesDialog(QtWidgets.QDialog):
    ...
    def __init__(self, relaxationzone, parent=None):
        ...
        self.table = QtWidgets.QTableWidget(50, 2)
    ...
    def fill_data(self):
        ...
        for row, value in enumerate(self.velocity_times):
            self.table.setItem(row, 0, QtWidgets.QTableWidgetItem(str(value[0])))
            self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(str(value[1])))
    ...
    def on_ok(self):
        ...
        for i in range(self.table.rowCount()):
            table_item_time: QtWidgets.QTableWidgetItem = self.table.item(i, 0)
            table_item_value: QtWidgets.QTableWidgetItem = self.table.item(i, 1)
            if table_item_time and table_item_value:
                self.velocity_times.append([table_item_time.value(),table_item_value.value()])
        self.accept()
