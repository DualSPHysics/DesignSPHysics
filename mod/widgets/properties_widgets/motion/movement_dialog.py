...
from mod.widgets.properties_widgets.motion.acc_circular_motion_timeline import AccCircularMotionTimeline
from mod.widgets.properties_widgets.motion.acc_rectilinear_motion_timeline import AccRectilinearMotionTimeline
from mod.widgets.properties_widgets.motion.acc_rotational_motion_timeline import AccRotationalMotionTimeline
from mod.widgets.properties_widgets.motion.adv_rotation_file_motion_timeline import AdvRotationFileMotionTimeline
from mod.widgets.properties_widgets.motion.cir_sinu_motion_timeline import CirSinuMotionTimeline
from mod.widgets.properties_widgets.motion.file_motion_timeline import FileMotionTimeline
from mod.widgets.properties_widgets.motion.focused_piston_wave_motion_timeline import FocusedPistonWaveMotionTimeline
from mod.widgets.properties_widgets.motion.irregular_flap_wave_motion_timeline import IrregularFlapWaveMotionTimeline
from mod.widgets.properties_widgets.motion.irregular_piston_wave_motion_timeline import IrregularPistonWaveMotionTimeline
from mod.widgets.properties_widgets.motion.movement_actions import MovementActions
from mod.widgets.properties_widgets.motion.movement_timeline_placeholder import MovementTimelinePlaceholder
from mod.widgets.properties_widgets.motion.path_file_motion_timeline import PathFileMotionTimeline
from mod.widgets.properties_widgets.motion.rect_sinu_motion_timeline import RectSinuMotionTimeline
from mod.widgets.properties_widgets.motion.rectilinear_motion_timeline import RectilinearMotionTimeline
from mod.widgets.properties_widgets.motion.regular_flap_wave_motion_timeline import RegularFlapWaveMotionTimeline
from mod.widgets.properties_widgets.motion.regular_piston_wave_motion_timeline import RegularPistonWaveMotionTimeline
from mod.widgets.properties_widgets.motion.rot_sinu_motion_timeline import RotSinuMotionTimeline
from mod.widgets.properties_widgets.motion.rotation_file_motion_timeline import RotationFileMotionTimeline
from mod.widgets.properties_widgets.motion.rotational_motion_timeline import RotationalMotionTimeline
from mod.widgets.properties_widgets.motion.solitary_piston_wave_motion_timeline import SolitaryPistonWaveMotionTimeline
from mod.widgets.properties_widgets.motion.wait_motion_timeline import WaitMotionTimeline
from mod.widgets.properties_widgets.motion.wave_movement_actions import WaveMovementActions
...
class MovementDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.target_mkbound=self.target.obj_mk
        self.mkbasedproperties = Case.the().get_mk_based_properties(self.target.real_mk())
        ...
        self.create_new_movement_menu.addAction(get_icon("regular_wave.png"), __("Solitary wave generator (Piston)"))
        ...
        self.create_new_movement_menu.addAction(get_icon("file_mov.png"), __("Advanced rotation from a file"))
        self.create_new_movement_menu.addAction(get_icon("file_mov.png"), __("Motion and rotation path from a file"))
        ...
        self.has_motion_targetlabel = QtWidgets.QLabel(__("Target MKBound: ") + str(self.target_mkbound))
        ...
        self.movement_list_groupbox = QtWidgets.QGroupBox(__("Global Movements")) #Primera columna
        self.movement_list_groupbox_layout = QtWidgets.QVBoxLayout()
        self.movement_list_groupbox.setMinimumWidth(380)
        self.movement_list_groupbox.setMaximumWidth(380)
        ...
        self.timeline_groupbox = QtWidgets.QGroupBox(__("Timeline for the selected movement")) #Segunda cloumna
        ...
        self.actions_groupbox = QtWidgets.QGroupBox(__("Available actions")) #Tercera columna
        self.actions_groupbox_layout = QtWidgets.QVBoxLayout()
        self.actions_groupbox.setMinimumWidth(250)
        self.actions_groupbox.setMaximumWidth(250)
        ...
        # Set motion susbcription for this mk
    ...
    def on_ok(self):
        ...
        info_dialog(__("This will apply the motion properties to all objects with mkbound = ") + str(self.target_mkbound))
    ...
    def on_check_movement(self, index, checked):
        ...
        target_movement = Case.the().info.global_movements[index]
        if checked:
    ...
    def on_loop_movement(self, index, checked):
        ...
        self.notice_label.setText("")  # Reset the notice label if a valid change is made
        Case.the().info.global_movements[index].set_loop(checked)

    def on_delete_movement(self, index):
        """ Remove a movement from the project. """
        try:
            self.movements_selected.remove(Case.the().info.global_movements[index])
    ...
    def on_new_wave_generator(self, action):
        ...
        if __("Advanced rotation from a file") in action.text():
            to_add = SpecialMovement(name="Advanced rotation from a file", generator=RotateAdvFileGen())
        if __("Motion and rotation path from a file") in action.text():
            to_add = SpecialMovement(name="Motion and rotation path from a file", generator=PathFileGen())
        if __("Solitary wave generator (Piston)") in action.text():
            to_add = SpecialMovement(name="Solitary Wave Generator (Piston)", generator=SolitaryPistonWaveGen())
    ...
    def on_movement_name_change(self, row, column):
        ...
        if target_item is not None and Case.the().info.global_movements[row].name != target_item.text():
            # Reset the notice label if a valid change is made
            self.notice_label.setText("")
    ...
    def on_movement_selected(self, row, _):
        ...
        try:
            self.currently_selected_movement = Case.the().info.global_movements[row]
        except IndexError:
            self.timeline_list_table.clearContents()
            self.timeline_list_table.setEnabled(False)
            self.timeline_list_table.setRowCount(1)
        ...
        if isinstance(self.currently_selected_movement, Movement):
            ...
            if isinstance(self.currently_selected_movement.generator, RegularFlapWaveGen):
                ...
                target_to_put = FileMotionTimeline(self.currently_selected_movement.generator, Case.the().path, parent=get_fc_main_window())
            elif isinstance(self.currently_selected_movement.generator, RotationFileGen):
                target_to_put = RotationFileMotionTimeline(self.currently_selected_movement.generator, Case.the().path, parent=get_fc_main_window())
            elif isinstance(self.currently_selected_movement.generator, RotateAdvFileGen):
                target_to_put = AdvRotationFileMotionTimeline(self.currently_selected_movement.generator, Case.the().path,
                                                       parent=get_fc_main_window())
            elif isinstance(self.currently_selected_movement.generator, PathFileGen):
                target_to_put = PathFileMotionTimeline(self.currently_selected_movement.generator, Case.the().path,
                                                       parent=get_fc_main_window())
            elif isinstance(self.currently_selected_movement.generator, SolitaryPistonWaveGen):
                target_to_put = SolitaryPistonWaveMotionTimeline(self.currently_selected_movement.generator, parent=get_fc_main_window())
